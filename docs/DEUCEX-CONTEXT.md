...
The design system is named Baseline and documented as a live page (`deucex-baseline.html`, section 2). The tokens and recipes below are what it was measured from.
...
`DeuceX.svg` (uploads) is inlined as `<symbol id="logo" viewBox="0 0 193 202">` and used in the sidebar header at 30px, the sign-in page and the onboarding header. It was removed from the topbar breadcrumb on 12 Sep ("remove this from all the files"): the topbar now starts with the page title.
