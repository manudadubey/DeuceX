...
## Deploy apps/api to Render — 26 September 2026

Owner decision: **Render**, not Fly.io. Render has no Tokyo region, so the service is in
**Singapore**, about 70 ms from the Supabase project in ap-northeast-1. That's roughly half the
round trip from Australia, not the 1 to 2 ms a same-region host would give.

Built:
- `render.yaml`, a Blueprint for one always-on web service, `deucex-api`:
  - Singapore region, `starter` plan. The free plan sleeps after 15 minutes, which would stop
    pg-boss and every scheduled job.
  - Node 22.13, `pnpm install --frozen-lockfile`, health check on `/health`.
  - Every secret is `sync: false`, so Render asks for it and it never lives in the repo.
  - `PATRON_LINK_SECRET` is generated by Render. `APP_BASE_URL` points at
    `https://deucex.vercel.app`.
- The API's `start` script is now `tsx src/index.ts`, and `tsx` is a runtime dependency. The old
  `node dist/index.js` could never have run: the shared `@deucex/*` packages are TypeScript
  source, so tsc's output imports them raw.

Verified locally with `NODE_ENV=production`:
- The API refuses to start while `ADMIN_PASSKEY_REQUIRED=false`.
- With the flag on, it starts; `/health` returns ok and `/admin/me` returns 401 without a
  session.

Needs the owner:
- A Render account and the paid plan.
- The secret values.
