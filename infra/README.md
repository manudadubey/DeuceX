...
- **apps/web** and **apps/admin** — Vercel, two separate deployments/hostnames. Admin never shares
  ...
  `deucex` (root `apps/web`) and `deucex-admin` (root `apps/admin`), team `md-labs`, both
...
- **Database** — Supabase project `gpzpmrumwaqyfkyvqbgl` ("ProCircuit", its pre-rebrand name, org `MD Labs`,
  `ap-northeast-1`). Shared with an unrelated pre-existing schema (matches/points/stats); RLS is on
  for those tables. DeuceX's own schema starts in build step 0.2.
...
- **GitHub** — [manudadubey/DeuceX](https://github.com/manudadubey/DeuceX), `main` branch.
  Not `matsudadubey/ProCircuit` — a different, unrelated account whose repo shares this one's pre-rebrand name.

Provisioned so far: GitHub, CI, both Vercel projects, the Supabase project (RLS fix only, no
DeuceX schema yet). Fly/Render and R2 are picked up when the steps that need them (0.6
onward) are reached.
